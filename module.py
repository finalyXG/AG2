...
def videogan_generator(self, image,z,mtx, options, reuse= False, name="generatorA"):
	...
	with tf.variable_scope(name):
		...
		i1 = lrelu(instance_norm(conv2d(i0, options.df_dim*2.0, 4, 2, name='ci_i1_conv_om'), 'ci_bni1_om'))
		i2 = lrelu(instance_norm(conv2d(i1, options.df_dim*2.0, 4, 2, name='ci_i2_conv_om'), 'ci_bni2_om'))
		i3 = lrelu(instance_norm(conv2d(i2, options.df_dim*4, 3, 2,name='ci_i3_conv_om'), 'ci_bni3_om'))
		i4 = lrelu(instance_norm(conv2d(i3, options.df_dim*4, 3, 2,name='ci_i4_conv_om'), 'ci_bni4_om'))
		i5 = lrelu(instance_norm(conv2d(i4, options.df_dim*4, 3, 2,name='ci_i5_conv_om'), 'ci_bni5_om'))
		...
		lin1 = tf.nn.relu(tf.contrib.layers.batch_norm(linear(lin1, 512, 'ci_lin1_om', with_w=False),scope="ci_bnlin1_om"))
		...
		i4_ = tf.tile(tf.expand_dims(i4,axis=1),[1,1,1,1,1])
		i3_ = tf.tile(tf.expand_dims(i3,axis=1),[1,1,1,1,1])
		i2_ = tf.tile(tf.expand_dims(i2,axis=1),[1,1,1,1,1])
		i1_ = tf.tile(tf.expand_dims(i1,axis=1),[1,1,1,1,1])
		i0_ = tf.tile(tf.expand_dims(i0,axis=1),[1,4,1,1,1])
		h0 = tf.concat([tf.reshape(h0, [-1, 1, 2, 2, 128]),i5_],axis=-1)
		...
		h1_2, self.h1_w_2, self.h1_b_2 = deconv3d(h1_1, [tf.shape(h0)[0], 2, 16, 16, 128],5,5,5,1,2,2,1,1,1, name='g_f_h1_2', with_w=True)
		...
		h1_3, self.h1_w_3, self.h1_b_3 = deconv3d(h1_2, [tf.shape(h0)[0], 2, 32, 32, 128],5,5,5,1,2,2,1,1,1, name='g_f_h1_3', with_w=True)
		...
		h1_4, self.h1_w_4, self.h1_b_4 = deconv3d(h1_3, [tf.shape(h0)[0], 4, 64, 64, 64],5,5,5,2,2,2,1,1,1, name='g_f_h1_4', with_w=True)
		...
		h1_5, self.h1_w_5, self.h1_b_5 = deconv3d(h1_4, [tf.shape(h0)[0], 4, 128, 128, 3],5,5,5,1,2,2,1,1,1, name='g_f_h1_5', with_w=True)

		mask = deconv3d(h1_4,[tf.shape(h0)[0], 4, 128, 128, 3],5,5,5,1,2,2,1,1,1, name='g_mask1', with_w=False)
		...
		image_tile = tf.tile(tf.expand_dims(image,axis=1),[1,options.frames_nb,1,1,1])
